...
var blob;
try {
  blob = new Blob([_slaveCode2['default']], { type: 'application/javascript' });
} catch (e) {
  // Backwards-compatibility
  var BlobBuilder = window.BlobBuilder || window.WebKitBlobBuilder || window.MozBlobBuilder;
  blob = new BlobBuilder();
  blob.append(_slaveCode2['default']);
  blob = blob.getBlob();
}
...
var Job = (function (_EventEmitter) {
  ...
  Job.prototype.executeOn = function executeOn(thread) {
    var _thread$off$on$once$once$run, _thread$off$on$once$once;

    this.emit('hasThread', thread);
    var that = this;
    (_thread$off$on$once$once$run = (_thread$off$on$once$once = thread.off('progress').on('progress', function (e) {
      that.emit('progress', e);
    }).once('message', function () {
      that.emit('done');
    }).once('error', this.emit.bind(this, 'error'))).run.apply(_thread$off$on$once$once, this.runArgs)).send.apply(_thread$off$on$once$once$run, this.sendArgs);
